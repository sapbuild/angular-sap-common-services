    ...
    function uiCommandManager($rootScope, $q, $log, $state, ActiveProjectService) {
        ...
        var applyCommand = function(command, actions) {
            ...
            return p
                ...
                .then(function(res) {
                    // cache result to return it later
                    // return command.postaction's promise here to make the outer promise resolve only once postaction is done
                    result = res;
                    return command.postaction(res);
                })
                .then(function() {
                    return result;
                })
                .catch(function(err) {
                    ...
                    if (!$state.is(command._state.name)) {
                        $state.go(command._state.name, command._state.params);
                    }
                });
        };
